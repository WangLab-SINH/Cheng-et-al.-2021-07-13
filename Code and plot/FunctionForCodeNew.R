...
MouseCalculateCost <- function(){
  ...
  load("EnergyCost/sequenceDNA.RData")
  ...
  load("EnergyCost/CC_RNA.RData")
  # 重新计算CC_RNA,WANG et al 的计算结果已保存在csv文件里
  matrixseqcostmouse = read.csv("EnergyCost/matrixseqcostmouse.csv",row.names = 1)
  ...
  # R_RNA，矩阵 ---------------------------------------------------
  i=9
  tmp = read.csv(paste0("EnergyCost/expression_amplitude/",
  ...
  # 统一id顺序，genename ----------------------------------------------
  # id对应表
  tmp = read.csv("EnergyCost/ampID.csv")
  ...
  load(file = 'EnergyCost/sequence_list_PRO_mouse.RData')
  ...
  rpzt = read.csv(
    "EnergyCost/RPZTrpkm.csv",
    row.names = 1)
  trzt = read.csv(
    "EnergyCost/TRZTrpkm.csv",
    row.names = 1)
...
YeastCalculateCost <- function(){
  ...
  # 重新计算CC_PRO,WANG et al 的计算结果已保存在csv文件里
  matrixseqcostyeast = read.csv("EnergyCost/matrixseqcostyeast.csv",row.names = 1)
  CC_RNA = matrixseqcostyeast$fulllength
  names(CC_RNA) = matrixseqcostyeast$ID
  # CM_RNA = 
  # LPRE_RNA，向量 ----------------------------------------------------
  tmp = read.table("EnergyCost/mart_export_transcript_length.txt",
                   header = T,sep = '\t')
  ...
  tmp = read.table("EnergyCost/yeast_mRNA_expression.txt",header = T)
  tmp1 = tmp$mRNA_Transcription_level
  names(tmp1) = tmp$Name
  tmp = read.table("EnergyCost/yeast_protein_abundance.txt",header = T)
  tmp2 = tmp$Protein_abundance
  names(tmp2) = toupper(tmp$ORF)
  tmp_id = names(tmp1)
  tmp = tmp2[tmp_id]/tmp1[tmp_id]
