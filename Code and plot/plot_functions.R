...
for(i in 1:(length(species) - 1))
{
  for(j in (i+1):length(species))
  {
    ...
    if (file.exists(current_file_name))
    {
      ...
      current_background_gene1_name <- paste0(paste0("Plot_required_file/background/", species[i]), "_background.txt")
      current_background_gene2_name <- paste0(paste0("Plot_required_file/background/", species[j]), "_background.txt")
    ...
    }else
    {
      ...
      current_background_gene1_name <- paste0(paste0("Plot_required_file/background/", species[j]), "_background.txt")
      current_background_gene2_name <- paste0(paste0("Plot_required_file/background/", species[i]), "_background.txt")
      current_background_gene1 <-read.table(current_background_gene1_name)$V1
      current_background_gene2 <-read.table(current_background_gene2_name)$V1
...
get_heatmap_function <- function(){
  ...
  draw_heatmap_function('All_meta2d_result/AllSpecies/Neurospora.csv',  24,35,"Neurospora")
  ...
  for(i in 1:(length(species)))
  {
    for(j in 1:length(species))
    {
      if(i != j)
      {
        ...
        current_file_gene2_name <- paste(paste("Plot_required_file/tissue14/enrich/",species[j],sep=""),".csv",sep="")
        gene1 <- read.csv(current_file_gene1_name,header = T)
        gene2 <- read.csv(current_file_gene2_name,header = T)
        for(k in 1:length(gene_name_table[[i]])){
          #temp_gene_name <- c()
